...
export default function DeviceDebugPanel({
  ...
  const [ip, setIp] = useState(getStoredDeviceIp());
  ...
  const handleConnect = () =>
    run('连接设备', async () => {
      const { info, status: next } = await deviceController.connect(ip);
      setStatus(next);
      onUpdateHardware({
        ...statusToHardwarePatch(next),
        connection: 'wifi',
        is_mock_mode: false,
      });
      setPolling(true);
      onLog(`[设备识别] ${info.name} ${info.model} v${info.version} @ ${next.ip || ip}`);
    });

  const handlePoll = () =>
    run('刷新状态', async () => {
      const next = await deviceController.pollOnce(ip);
      setStatus(next);
      onUpdateHardware(statusToHardwarePatch(next));
    });
  ...
  const handleTherapy = () =>
    run('下发治疗参数', async () => {
      ...
      const next = await deviceController.pollOnce(ip);
      setStatus(next);
      onUpdateHardware({ ...statusToHardwarePatch(next), is_running: true });
    });

  const handleStop = () =>
    run('急停', async () => {
      await deviceController.stop(ip);
      const next = await deviceController.pollOnce(ip);
      setStatus(next);
      onUpdateHardware({ ...statusToHardwarePatch(next), is_running: false });
    });

  const handleReset = () =>
    run('解除软件急停', async () => {
      await deviceController.reset(ip);
      const next = await deviceController.pollOnce(ip);
      setStatus(next);
      onUpdateHardware(statusToHardwarePatch(next));
    });
  ...
  const handleTare = () =>
    run('力传感器去皮', async () => {
      await deviceController.tare(ip);
      const next = await deviceController.pollOnce(ip);
      setStatus(next);
      onUpdateHardware(statusToHardwarePatch(next));
    });
        ...
        <div className="fixed inset-x-3 bottom-36 z-[80] max-h-[62vh] overflow-y-auto rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-2xl backdrop-blur md:inset-x-auto md:right-4 md:w-[360px]">
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-500">文档：docs/API-设备HTTP接口.md</p>
            </div>
            <button
              type="button"
