...
export default function PatientApp({
  ...
  useEffect(() => {
    ...
    patientProfile.id,
    patientProfile.symptoms_assessed,
    patientProfile.onboarding_completed,
    patientProfile.auth_code_used,
    patientProfile.current_prescription,
  ]);

  // 连接设备后：无方案则留在自评/匹配流程，不自动进入控制面板；已填报症状则不重新弹出冷启动
  useEffect(() => {
    if (!isHardwareLinked) return;
    if (hasTreatmentPlan(patientProfile)) return;

    const symptomsReady =
      patientProfile.symptoms_assessed === true || isSymptomComplete(symptomForm);

    if (symptomsReady) {
      setIsColdBootActive(false);
      setTherapyStep((step) => (step === 'control' ? 'symptom' : step));
      return;
    }
    ...
    patientProfile.symptoms_assessed,
    symptomForm.age,
    symptomForm.cartilage_wear,
    symptomForm.joint_fluid,
    symptomForm.pain_score,
  ]);
  ...
  const handleAcceptDoctorParams = (params: TreatmentParams) => {
    ...
    setTherapyStep('control'); // Slices to dashboard console
  ...
  const handleToggleTherapy = () => {
    if (hardwareState.connection === 'disconnected') {
  ...
  const handleConfirmPrescriptionUpdate = async () => {
    ...
    try {
      ...
      onSendHardwareAction(
        `[医嘱置顶覆盖] 用户确认接受医生处方：L=${rx.left_force}N, R=${rx.right_force}N, T=${rx.temp}℃`
      );
    } catch (err) {
  ...
  const handleConnectionToggle = () => {
    ...
    if (isHardwareLinked) {
      ...
      onConnectDevice?.('bluetooth');
      onSendHardwareAction('[蓝牙切换] 用户发起蓝牙连接请求，进入配对流程');
    }
  };

  const handleTransportConnect = (transport: HardwareTransport) => {
    if (isConnecting) return;
    if (isHardwareLinked && hardwareState.connection === transport) return;
    onConnectDevice?.(transport);
    onSendHardwareAction(
            ...
            <div className="bg-white rounded-3xl p-4 border border-slate-200/80 shadow-sm flex flex-col gap-2 shrink-0">
              ...
              <div className="grid grid-cols-2 gap-2 mt-1">
                ...
                >
                  ...
                  家庭 Wi-Fi 绑定
                  <span className="text-[8px] font-light text-slate-400">远程同步 异地数据不漏</span>
                </button>
              </div>
            </div>
